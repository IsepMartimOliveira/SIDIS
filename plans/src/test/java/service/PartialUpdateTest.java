...
public class PartialUpdateTest {
    ...
    @Test
    public void partialUpdateTest() throws IOException, URISyntaxException, InterruptedException {
        ...
        when(plansRepository.findByName(plan.getName().getName())).thenReturn(Optional.of(plan));
        when(plansRepository.save(Mockito.any(Plans.class))).thenReturn(plan);


        Plans result = plansService.partialUpdate(plan.getName().getName(), request, "sdfsfdfsfsdf",0L);
    ...
    public void promoteTest_PlanDoesNotExist() {
        ...
        assertThrows(IllegalArgumentException.class, () -> plansService.partialUpdate(planName, editPlansRequest,"sdfsfdfsfsdf",0L));
    }
